    ...
    return (
        <Motion.header
            ...
            className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${isScrolled
                ? "backdrop-blur-2xl bg-white/90 shadow-xl shadow-black/5 border-b border-black/10"
                : "bg-transparent"
                }`}
        >
            <div className="container-grid flex items-center justify-between py-5 lg:py-4">
                ...
                <nav className="hidden items-center gap-1 lg:flex">
                    {navItems.map((item, index) => (
                        <NavLink
                            ...
                            to={item.path}
                            className={({ isActive }) =>
                                `relative px-5 py-2.5 font-semibold text-sm transition-all duration-300 rounded-full group ${isActive
                ...
                <Motion.button
                    whileTap={{ scale: 0.9 }}
                    className={`lg:hidden p-2.5 rounded-xl transition-all duration-300 ${isScrolled ? "bg-accent/10 text-accent" : "bg-white/20 text-primary backdrop-blur-sm"
                        }`}
                    type="button"
